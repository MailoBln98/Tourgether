...
const RouteDetailScreen: React.FC = () => {
    ...
    const refetchRoute = async () => {
        ...
        if (routeResponse.ok) {
            ...
            const updatedRoute = data.find(r => r._id === routeId);
            if (updatedRoute) {
                setRoute(updatedRoute);
                fetchUserNames(updatedRoute.registered_users);
                setIsRegistered(updatedRoute.registered_users.includes(currentUserId));
                return updatedRoute;
            }
        }
        return null;
